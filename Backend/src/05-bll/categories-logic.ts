...
async function deleteCategory(_id: string) {
    ...
    const prodByCatId = await getProductsByCategoryId(_id);
    if(prodByCatId.length !== 0) throw new ClientError(400, "There are still some products in that category, you cannot delete it until its empty");
